...
If the daily run fails with transient errors (for example, API rate limits), it retries up to 3 attempts by default.

Optional environment variables:

- `DAILY_RETRY_MAX_ATTEMPTS` → max daily attempts (default `3`; set `0` to retry indefinitely until success or external cancellation/workflow timeout)
